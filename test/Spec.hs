...
spec =
  describe "Antikythera" $ do
    ...
    describe "Utils" $ do
      describe "nextPeriods" $ do
        describe "at 5 hour .&& untilInclusive" $ do
          it "before the deadline" $ do
            let p = at 5 hour .&& untilInclusive (addUTCTime nominalDay) (time "2025-03-17T04:05:00Z")
            (iso8601Show <$> nextPeriods p (time "2025-03-15T02:01:50Z"))
              `shouldBe` ["2025-03-15T05:00:00Z", "2025-03-16T05:00:00Z"]
          it "after the deadline" $ do
            let p = at 5 hour .&& untilInclusive (addUTCTime nominalDay) (time "2025-03-17T04:05:00Z")
            (iso8601Show <$> nextPeriods p (time "2025-03-18T02:01:50Z"))
              `shouldBe` []
